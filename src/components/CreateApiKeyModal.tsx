...
const CreateApiKeyModal = ({ routes, onCreate, formError }: CreateApiKeyModalProps) => {
  const { control, register, handleSubmit, watch, reset, setValue, formState: { errors, isValid, isSubmitting } } = useForm<CreateKeyFormValues>({
    ...
    <dialog id="create_apikey_modal" className="modal">
      <form className="modal-box max-w-md mx-auto" onSubmit={handleSubmit(onSubmit)}>
        ...
        <fieldset className="fieldset">
          ...
          {errors.permissions && <p className="text-error text-sm mt-1">{errors.permissions.message}</p>}
        </fieldset>
        <div className="modal-action">
          <button
            ...
            onClick={
              () => { (document.getElementById("create_apikey_modal") as HTMLDialogElement).close(); }
            }
          >
            Cancel
          </button>
