...
export const TopNav = (): JSX.Element => {
  ...
  const visibleDropdownLinks = filterAndSortRoutes(navRoutes, ['Profile', 'API Keys'], user)
    .filter(({ isPublic = false }) => {
      if (isPublic && hideNavPaths.includes(location.pathname)) return false
      return true;
  })
...
*/}      {/*Sidebar Toggle*/}
      <div className="navbar-start">
        <NavLink to="/">  {/* add className="xl:hidden" for sidebar */}
          <svg className="text-base-content" width="140" height="40" viewBox="0 0 200 50" xmlns="http://www.w3.org/2000/svg">
            <image
              href={AppConfig.logo}
              x="0"
              y="0"
          ...
          */}
          ...
          {user ? (
            <div className="dropdown dropdown-end">
              ...
              <ul tabIndex={0} className="menu dropdown-content bg-base-100 rounded-box z-1 w-35 p-2 shadow-sm">
                {visibleDropdownLinks.map(({ label, path }) => (
                  <li key={`vislink_${String(label)}`}>
                    <NavLink onClick={() => void document.activeElement.blur()} to={path}>{label}</NavLink>
                  </li>
                ))}
                <li key="logout">
            ...
            <NavLink className="btn btn-ghost px-2" to="/login">Login</NavLink>
          )}
        </div>
      </div>
    </div>
