...
export const refreshTokenThunk = createAsyncThunk<{ token: string }, void, { dispatch: AppDispatch; getState: () => RootState }>(
  "auth/refresh",
  async (_, thunkAPI) => {
    try {
...
// eslint-disable-next-line @typescript-eslint/no-invalid-void-type 
export const logoutThunk = createAsyncThunk<void, object, { getState: () => RootState }>(
  "auth/logout",
  async (_, thunkAPI) => {
    try {
...
// eslint-disable-next-line @typescript-eslint/no-invalid-void-type
export const fetchUserProfileThunk = createAsyncThunk<User, void, { dispatch: AppDispatch; getState: () => RootState; rejectValue: string }>(
...
export const updateUserProfileThunk = createAsyncThunk<User, { email: string; full_name: string }, { dispatch: AppDispatch; getState: () => RootState; rejectValue: string }>(
  "auth/updateUser",
...
// eslint-disable-next-line @typescript-eslint/no-invalid-void-type
export const updateUserProfilePasswordThunk = createAsyncThunk<void, { current_password: string; new_password: string }, { dispatch: AppDispatch; getState: () => RootState; rejectValue: string }>(
  ...
  async ({ current_password, new_password }, thunkAPI) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        return thunkAPI.rejectWithValue(err.message || "Network error while changing password");
      }
    }
  }
);
