...
// Fetch keys
export const fetchApiKeysThunk = createAsyncThunk<
  ApiKey[],
  undefined,
  ThunkConfig
>("apikeys/fetchAll", async (_, thunkAPI) => {
  try {
    const response = await fetchWithAuth("/apikeys", {}, thunkAPI)
    if (!response.ok)
      return thunkAPI.rejectWithValue(
        await extractError(response, "Failed to fetch API keys"),
      )
...
export const fetchRoutesThunk = createAsyncThunk<
  ApiRoute[],
  undefined,
  ThunkConfig
>("apikeys/fetchRoutes", async (_, thunkAPI) => {
  try {
    const response = await fetchWithAuth("/apikeys/routes", {}, thunkAPI)
    if (!response.ok)
      return thunkAPI.rejectWithValue(
        await extractError(response, "Failed to fetch routes"),
      )
...
export const createApiKeyThunk = createAsyncThunk<
  ...
  ThunkConfig
>("apikeys/create", async (payload, thunkAPI) => {
  try {
    ...
    if (!response.ok)
      return thunkAPI.rejectWithValue(
        await extractError(response, "Failed to create key"),
      )
    return (await response.json()) as ApiKey
  } catch (err) {
    return thunkAPI.rejectWithValue(
...
export const fetchApiKeyRoutesThunk = createAsyncThunk<
  Permission[],
  string,
  ThunkConfig
>("apikeys/fetchKeyRoutes", async (id, thunkAPI) => {
  try {
    const response = await fetchWithAuth(`/apikeys/${id}/routes`, {}, thunkAPI)
    if (!response.ok)
      return thunkAPI.rejectWithValue(
        await extractError(response, "Failed to fetch key routes"),
      )
    return (await response.json()) as Permission[]
  } catch (err) {
    return thunkAPI.rejectWithValue(
      err instanceof Error ? err.message : "Failed to fetch key routes",
    )
  }
})
...
export const revokeApiKeyThunk = createAsyncThunk<string, string, ThunkConfig>(
  ...
  async (id, thunkAPI) => {
    try {
      ...
      if (!response.ok)
        return thunkAPI.rejectWithValue(
          await extractError(response, "Failed to revoke key"),
        )
...
export const reissueApiKeyThunk = createAsyncThunk<
  ApiKey,
  { id: string; expiresAt: string | null },
  ThunkConfig
>("apikeys/reissue", async ({ id, expiresAt }, thunkAPI) => {
  try {
    const response = await fetchWithAuth(
      `/apikeys/${id}`,
      {
    ...
    if (!response.ok)
      return thunkAPI.rejectWithValue(
        await extractError(response, "Failed to reissue key"),
      )
    ...
    return thunkAPI.rejectWithValue(
      err instanceof Error ? err.message : "Failed to reissue key",
    )
  }
})
...
export const deleteApiKeyThunk = createAsyncThunk<string, string, ThunkConfig>(
  ...
  async (id, thunkAPI) => {
    try {
      ...
      if (!response.ok)
        return thunkAPI.rejectWithValue(
          await extractError(response, "Failed to delete key"),
        )
      await thunkAPI.dispatch(fetchApiKeysThunk())
      return id
    } catch (err) {
